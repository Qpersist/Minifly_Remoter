...
{	
	...
	SPI_NRF_WriteBuf(WR_TX_PLOAD, tx_buf, len); 			 // 装载数据	
...
void Nrf24l01_Init(u8 model, u8 ch)
{
	...
	SPI_NRF_WriteReg(NRF_WRITE_REG+EN_AA,0x01); 													//使能通道0的自动应答 
	SPI_NRF_WriteReg(NRF_WRITE_REG+EN_RXADDR,0x01);											//使能通道0的接收地址 
	SPI_NRF_WriteReg(NRF_WRITE_REG+SETUP_RETR,0x1a);											//设置自动重发间隔时间:500us;最大自动重发次数:10次 
	SPI_NRF_WriteReg(NRF_WRITE_REG+RF_CH,80);														//设置RF通道为CHANAL
	SPI_NRF_WriteReg(NRF_WRITE_REG+RF_SETUP,0x0f); 												//设置TX发射参数,0db增益,2Mbps,低噪声增益开启
...
/////////////////////////////////////////////////////////
	...
	{
		SPI_NRF_WriteReg(NRF_WRITE_REG+RX_PW_P0,RX_PLOAD_WIDTH);								//选择通道0的有效数据宽度 
		SPI_NRF_WriteReg(NRF_WRITE_REG + CONFIG, 0x0f);   		 // IRQ收发完成中断开启,16位CRC,主接收
	}
	else if(model==2)		//TX
	{
		SPI_NRF_WriteReg(NRF_WRITE_REG+RX_PW_P0,RX_PLOAD_WIDTH);								//选择通道0的有效数据宽度 
		SPI_NRF_WriteReg(NRF_WRITE_REG + CONFIG, 0x0e);   		 // IRQ收发完成中断开启,16位CRC,主发送
	}
	else if(model==3)		//RX2
	{
